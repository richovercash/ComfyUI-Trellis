...
import tempfile
import logging
import torch  # Add torch import for tensor handling
...
class TrellisClientComfy:
    ...
    async def connect(self):
        ...
        try:
            ...
            # Try with expanded parameters and explicit protocol
            self.websocket = await websockets.connect(
                url,
                ping_interval=30,
                ping_timeout=30,
                close_timeout=60,  # Extended timeout
                max_size=None,
                compression=None,  # Disable compression to simplify handshake
                origin="http://localhost:8188",  # Add origin header (may help with handshake)
                extra_headers={
                    "User-Agent": "ComfyUI-Trellis/0.1.0",
                    "Connection": "Upgrade",
                    "Upgrade": "websocket"
                }
            )
